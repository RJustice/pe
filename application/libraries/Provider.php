...
abstract class OAuth2_Provider
{
	...
	public function __construct(array $options = array())
	{
		...
		$this->client_id = $options['appkey'];
		
		isset($options['callback']) and $this->callback = $options['callback'];
		isset($options['secretkey']) and $this->client_secret = $options['secretkey'];
			...
			case 'POST':
				$opts = array(
					'http' => array(
        				...
        				$rest_config = get_instance()->config->load('rest');
				get_instance()->load->library('rest', array('server'=>'https://oauth.taobao.com/'));
				get_instance()->rest->option(CURLOPT_SSL_VERIFYPEER,FALSE);
				//var_dump(get_instance()->rest->post($url,$params));
				//var_dump($params);exit;
				$return = get_instance()->rest->post('token',$params,'json');
				$return  = (array) $return;
				//var_dump($return);
				//var_dump($response);
